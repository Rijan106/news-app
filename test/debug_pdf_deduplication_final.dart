...
List<String> _removeDuplicatePdfs(List<String> pdfUrls) {
  ...
  for (final url in pdfUrls) {
    ...
    // For encoded URLs, extract the decoded version for comparison
    String comparisonKey = normalizedUrl;
    if (url.contains('pdfID=') && url.contains('url=')) {
      try {
        ...
        if (urlParam != null) {
          final decodedUrl = Uri.decodeFull(urlParam);
          final normalizedDecoded = _normalizePdfUrl(decodedUrl);
          comparisonKey = normalizedDecoded; // Use decoded version as key
        }
      } catch (e) {
    ...
    // Only store if we haven't seen this normalized URL before
    if (!normalizedUrls.containsKey(comparisonKey)) {
      normalizedUrls[comparisonKey] = url;
    }
  }
